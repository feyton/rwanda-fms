$(document).ready(function () {
    ...
    $(function () {
        ...
        function addMessageHtml(fromUser, message) {
            let $message = $("#messages");

            if (fromUser === username) {
                $message.append('<li class="timeline-inverted"><div class="timeline-badge success"><i class="now-ui-icons users_single-02"></i></div><div class="timeline-panel"><div class="timeline-heading"><span class="badge badge-primary">' + fromUser + '</span></div><div class="timeline-body"><p>' + message + '</p></div></div></li>')
            } else {
                $message.append('<li ><div class="timeline-badge warning"><i class="now-ui-icons users_single-02"></i></div><div class="timeline-panel"><div class="timeline-heading"><span class="badge badge-info">' + fromUser + '</span></div><div class="timeline-body"><p>' + message + '</p></div></div></li>')
            }
        ...
        $.getJSON(
            ...
            function (data) {
                ...
                $("#username_assign").text("You have been assigned a random username of: " + username)
                // print(
        ...
        function setupChannel(name) {
            roomChannel.join().then(function (channel) {
                $("#room_joining").text("Joined as " + username);
                $("#username_assign").hide();
            ...
            roomChannel.on("messageAdded", function (message) {
                ...
                addMessageHtml(message.author, message.body)
            });
        }
        ...
        function createOrJoinChannel(channels) {
            ...
            //print(`Attempting to join "${channelName.toUpperCase()}" chat channel...`);
            $("#room_joining").text("Attempting to join...");
